...
// 修复的核心部分 - 确保所有IP节点都能显示
  const generateIPTable = (start, end, isTLS) => {
    ...
    for (let i = start; i <= end; i++) {
      ...
      const port = eval(`PT${i}`);
      const type = isTLS ? "TLS" : "HTTP";
      const protocol = isTLS ? "https" : "http";
      
      rows += `
      <tr>
        <td>CF_V${i}_${ip}_${port}</td>
        <td class="limited-width">${protocol}://${ip}:${port}</td>
        <td><button class="btn btn-primary" onclick="copyToClipboard('${protocol}://${ip}:${port}')">复制</button></td>
      </tr>`;
    }
    return rows;
  };
  ...
  <head>
    ...
    <style>
      .limited-width { max-width: 300px; overflow-wrap: break-word; }
    </style>
    <script>
  ...
  <body>
    <div class="container">
      <h1>Cloudflare代理配置</h1>
      
      <h3>HTTP节点 (IP1-IP7)</h3>
      <table class="table">
        <thead>
          <tr><th>节点名称</th><th>地址</th><th>操作</th></tr>
        </thead>
        <tbody>
          ${generateIPTable(1, 7, false)}
        </tbody>
      </table>
      
      <h3>HTTPS节点 (IP8-IP13)</h3>
      <table class="table">
        <thead>
          <tr><th>节点名称</th><th>地址</th><th>操作</th></tr>
        </thead>
        <tbody>
          ${generateIPTable(8, 13, true)}
        </tbody>
      </table>
    </div>
  </body>`;
